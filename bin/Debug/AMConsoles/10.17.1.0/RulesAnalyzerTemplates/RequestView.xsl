...
<xsl:param name="aaduser">AAD User</xsl:param>
<xsl:param name="aadgroup">AAD Group</xsl:param>  
  ...
  <table width="100%" bgcolor="#E0E0E0" style="COLOR:#C0C0C0;BORDER:0">
    <tr>
      <td width="5%" style="FONT:10pt Verdana,sans-serif;COLOR:#707070;BACKGROUND:#E0E0E0">
        ...
        <xsl:when test="@type = 'AADUser'">
          <img border="0" src="./Images/AADUser_16.png" width="16" height="16"></img>
        </xsl:when>	          
        <xsl:when test="@type = 'AADGroup'">
          <img border="0" src="./Images/aadGroupRule_16.png" width="16" height="16"></img>
        </xsl:when>	                    
      ...
      <td style="FONT:10pt Verdana,sans-serif;COLOR:#0000C4;CURSOR:hand;BACKGROUND:#E0E0E0;TEXT-DECORATION:underline">
        ...
        <xsl:choose>
          ...
          <xsl:when test="@type = 'AADUser'">
            <xsl:value-of select="$aaduser"/>
          </xsl:when>
          <xsl:when test="@type = 'AADGroup'">
            <xsl:value-of select="$aadgroup"/>
          </xsl:when>          
          <xsl:otherwise>
            <xsl:value-of select="@type"/>
          </xsl:otherwise>
