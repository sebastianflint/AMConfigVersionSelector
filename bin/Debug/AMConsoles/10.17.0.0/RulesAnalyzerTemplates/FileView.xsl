...
<xsl:param name="filterIncludeOverwriteRenameRequests">true</xsl:param>
...
<xsl:param name="filterOverwriteRenameCheckboxText">Include file Overwrite and Rename requests</xsl:param>  
    ...
    <xsl:otherwise>
      ...
      <table width="100%">
        <xsl:choose>
          ...
          <xsl:when test="$display-filter = 'time'">
            <xsl:if test="($filterIncludeOverwriteRenameRequests='true') or ((@type!='Overwrite') and (@type!='Rename'))">
              <tr>
              ...
              <xsl:call-template name="IndividualRequestSummaryType"/>
              <tr>
                <td width="2.5%"></td>
          ...
          <!-- Display only the allowed requests for a particular user -->
          <xsl:when test="$display-filter = 'user-allowed'">
            <xsl:if test="$user-filter = ./user/@sid">
              <xsl:if test="(@result='Allow') or (@result = 'User Rights Modified')">
                <xsl:if test="($filterIncludeOverwriteRenameRequests='true') or ((@type!='Overwrite') and (@type!='Rename'))">
          ...
          <!-- Display only the denied requests for a particular user -->
          <xsl:when test="$display-filter = 'user-denied'">
            <xsl:if test="$user-filter = ./user/@sid">
              <xsl:if test="@result='Deny'">
                <xsl:if test="($filterIncludeOverwriteRenameRequests='true') or ((@type!='Overwrite') and (@type!='Rename'))">
          ...
          <!-- Display only the allowed user rights requests for a particular user -->
          <xsl:when test="$display-filter = 'allowed-user-rights-modified'">
            <xsl:if test="$user-filter = ./user/@sid">
              <xsl:if test="@result='User Rights Modified'">
                <tr>
...
<xsl:template name="IndividualRequestSummaryType">
  <!-- ================================================================================ -->
  <!-- Display the Type of an individual Request in the Requests view					  -->
  <!-- ================================================================================ -->
  <xsl:choose>
    <!--As per the individual RequestView we only show Type for non-browser requests-->
    <xsl:when test="@type != 'Browser'">
      <tr>
        <td width="2.5%"></td>
        <td>
...
<xsl:template name="Default">
  <!-- Obtain the node-set to work with, applying any filtering -->
  <xsl:variable name="workingNodeSet" select="
                                    /Explanations/log/request[(@type!='Overwrite') and (@type!='Rename')] 
                                    | 
                                    /Explanations/log/request[($filterIncludeOverwriteRenameRequests='true') and ((@type='Overwrite') or (@type='Rename'))]"/>
  ...
  <table bgcolor="#E0E0E0" width="100%">
    <tr>
      <td class="small-border"><xsl:value-of select="$workingNodeSet[1]/@time"/>
        &#160;<xsl:value-of select="$on" />&#160; 
        <xsl:value-of select="$workingNodeSet[1]/@date"/></td>
      <td class="small-border"><xsl:value-of select="$workingNodeSet[last()]/@time"/>
        &#160;<xsl:value-of select="$on" />&#160;
        <xsl:value-of select="$workingNodeSet[last()]/@date"/></td>
    </tr>
  </table>
  <br/>
  
  <!-- Request table info label-->
  <table width="100%">
    <tr>
    <td width="100%">
  ...
  <!-- Include overwrite rename requests check box-->
  <label width="100%">
    <input type='checkbox' onclick='handleFilterIncludeOverwriteRenameClick(this)'>
      <xsl:if test="$filterIncludeOverwriteRenameRequests='true'">
        <xsl:attribute name="checked"/>
      </xsl:if>
    </input>
    <xsl:value-of select="$filterOverwriteRenameCheckboxText" />
  </label> 
  <br/>
  <br/>
  ...
  <table class="border-top-left" width="100%">
    ...
    <tr>
      ...
      <td class="small-border-trigger-one">
        ...
        <xsl:value-of select="count($workingNodeSet)"/>
      </td>
      <td class="small-border-trigger-one">
        <xsl:attribute name="onClick">
          showRequests("allowed", "none");
        </xsl:attribute>
        <xsl:value-of select="count($workingNodeSet[@result='Allow']) + count($workingNodeSet[@result='User Rights Modified'])"/>
      </td>      
      <td class="small-border-trigger-one">
        <xsl:attribute name="onClick">
          showRequests("denied", "none");
        </xsl:attribute>
        <xsl:value-of select="count($workingNodeSet[@result='Deny'])"/>
      </td>		
      <td class="small-border-trigger-one">
        <xsl:attribute name="onClick">
          showRequests("user rights modified", "none");
        </xsl:attribute>
        <xsl:value-of select="count($workingNodeSet[@result='User Rights Modified'])"/>
      </td>
      <td class="small-border-trigger-one">
        <xsl:attribute name="onClick">
          showRequests("browser", "none");
        </xsl:attribute>
        <xsl:value-of select="count($workingNodeSet[@type='Browser'])"/>
      </td>
    </tr>

    <!-- 
    Per-user rows.
    To find the unique SID keys we must use the key method, which evaluates the whole document rather than our filtered
    list. this will just mean we show users with no requests. not a terrible outcome and less complex than trying to 
    update the key to do a filtered search too.
    -->
    <xsl:for-each select="/Explanations/log/request/user[generate-id(.)=generate-id(key('users', @sid))]/@sid">
    <tr>
      ...
      <td class="small-border-trigger-one">
        ...
        <xsl:value-of select="count($workingNodeSet/user[@sid=$user_sid])"/>
      </td>          
      <td class="small-border-trigger-one">
        <xsl:attribute name="onClick">
          showRequests("user-allowed", '<xsl:value-of select="../@sid"/>');
        </xsl:attribute>
        <xsl:value-of select="count($workingNodeSet[@result='Allow']/user[@sid=$user_sid]) + count($workingNodeSet[@result='User Rights Modified']/user[@sid=$user_sid])"/>
      </td>      
      <td class="small-border-trigger-one">
        <xsl:attribute name="onClick">
          showRequests("user-denied", '<xsl:value-of select="../@sid"/>');
        </xsl:attribute>
        <xsl:value-of select="count($workingNodeSet[@result='Deny']/user[@sid=$user_sid])"/>
      </td>
      <td class="small-border-trigger-one">
        <xsl:attribute name="onClick">
          showRequests("allowed-user-rights-modified", '<xsl:value-of select="../@sid"/>');
        </xsl:attribute>
        <xsl:value-of select="count($workingNodeSet[@result='User Rights Modified']/user[@sid=$user_sid])"/>
      </td>
      <td class="small-border-trigger-one">
        <xsl:attribute name="onClick">
          showRequests("user-browser", '<xsl:value-of select="../@sid"/>');
        </xsl:attribute>
        <xsl:value-of select="count($workingNodeSet[@type='Browser']/user[@sid=$user_sid])"/>
      </td>
