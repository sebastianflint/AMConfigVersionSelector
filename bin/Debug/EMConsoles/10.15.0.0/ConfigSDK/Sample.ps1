...
write "Adding Certification Authentication thumbprints"

# -- Add thumbprints for client Certificate Authentication
$expiry = Get-Date -Date "2035-05-01 00:00:00Z"
$expiry.ToUniversalTime()
$config.AddCertificateAuthenticationThumbprint("CA Certificate", "4066A033C497FC2B9C2F238BEB941D197AEF88B0", $expiry)
$config.AddCertificateAuthenticationThumbprint("Another CA Certificate", "18753FD73B943A900C2F238BEB941D19767D3211", $expiry)

write "Testing validation for adding Cetification Authentication thumbprints"

try
{
    $config.AddCertificateAuthenticationThumbprint("", "1111111111111111111111111111111111111111", $expiry)

    throw "Should have been a 'subject' exception"

}
catch
{
    if (!$_.Exception.Message.Contains("Subject"))
    {
        throw "Should have been a 'subject' exception: " + $_.Exception.Message
    }
}

try
{
    $config.AddCertificateAuthenticationThumbprint("invalid", "invalidinvalidinvalidinvalidinvalidinval", $expiry)

    throw "Should have been a 'thumbprint' exception"

}
catch
{
    if (!$_.Exception.Message.Contains("thumbprint"))
    {
        throw "Should have been a 'thumbprint' exception: " + $_.Exception.Message
    }
}

try
{
    $config.AddCertificateAuthenticationThumbprint("too short", "123456789012345678901234567890123456789", $expiry)
...
{
    $expiry = Get-Date
    $expiry = $expiry.AddSeconds(-1)
    $config.AddCertificateAuthenticationThumbprint("Expired", "1111111111111111111111111111111111111111", $expiry)

    throw "Should have been an 'expired' exception"

}
catch
{
    if (!$_.Exception.Message.Contains("expired"))
    {
        throw "Should have been an 'expired' exception: " + $_.Exception.Message
    }
}
...
{
    $expiry = Get-Date
    $expiry = $expiry.AddDays(30)
    $expiry.ToUniversalTime()
    $config.AddCertificateAuthenticationThumbprint("Expiring", "1111111111111111111111111111111111111111", $expiry)

    throw "Should have been an 'expiring' exception"

}
catch
{
    if (!$_.Exception.Message.Contains("expires"))
    {
        throw "Should have been an 'expiring' exception: " + $_.Exception.Message
    }
}
...
# AccessControlList action
# Allow on read & execute, deny on write.
# Apply to WD SID which is everyone.
write "Access Control List"
$aclUser = [EMConfigAPI.Actions.AccessControlList+UserType]::User
$aclGroup = [EMConfigAPI.Actions.AccessControlList+UserType]::Group
$aclNone = [EMConfigAPI.Actions.AccessControlList+AceAccessType]::None
$aclAllow = [EMConfigAPI.Actions.AccessControlList+AceAccessType]::Allow
$aclDeny = [EMConfigAPI.Actions.AccessControlList+AceAccessType]::Deny

$acls = New-Object Collections.Generic.List[EMConfigAPI.Actions.AccessControlList+AccessControlEntry]
$everyoneAcl = New-Object EMConfigAPI.Actions.AccessControlList+AccessControlEntry("WD", $aclGroup, $aclNone, $aclNone, $aclAllow, $aclAllow, $aclDeny)
$systemAcl = New-Object EMConfigAPI.Actions.AccessControlList+AccessControlEntry("SY", $aclGroup, $aclAllow, $aclNone, $aclNone, $aclNone, $aclNone)
$userAcl = New-Object EMConfigAPI.Actions.AccessControlList+AccessControlEntry("George Stephenson", "gstephenson@rocket.com", "S-1-5-21-1234567890-1234567890-1234567890", $aclUser, $aclDeny, $aclDeny, $aclDeny, $aclDeny, $aclDeny)

$acls.Add($everyoneAcl)
$acls.Add($systemAcl)
$acls.Add($userAcl)

 [Void] $config.AddActionOrCondition($fileActionsId, (New-Object EMConfigAPI.Actions.AccessControlList("C:\config.renamed", $acls, $true)))
...
# -- Create url shortcut action
$createUrlShortcut = New-Object EMConfigAPI.Actions.CreateUrlShortcut("%USERPROFILE%\Desktop\My BBC.url", $false, "http://www.google.com")
[void] $config.AddActionOrCondition($shortcutActionId, $createUrlShortcut)
...
# --- Computer Is Aad Joined --
write "ComputerIsAadJoined"
$ComputerIsAadJoined = $config.AddNodeToParent($preDesktopConditions, "Computer Is AAD Joined")
[void] $config.AddActionOrCondition($ComputerIsAadJoined,(new-object EMConfigAPI.Conditions.ComputerIsAadJoined($true)))
...
$customConditionObject = new-object EMConfigAPI.Conditions.CustomCondition("Powershell", $text, "%TEMP%", "System", 10, 1, 1)
...
write "File Metadata Condition"
$metadataEntries = New-Object Collections.Generic.List[EMConfigAPI.Conditions.FileMetadata+MetadataEntry]
$attributeEntry = New-Object EMConfigAPI.Conditions.FileMetadata+MetadataEntry("Attribute", "A", "Equal")
$existsEntry = New-Object EMConfigAPI.Conditions.FileMetadata+MetadataEntry("Bar", "", "Exist")
$notExistsEntry = New-Object EMConfigAPI.Conditions.FileMetadata+MetadataEntry("Foo", "", "NotExist")
$nameEntry = New-Object EMConfigAPI.Conditions.FileMetadata+MetadataEntry("Name", "C:\*", "Query")
$metadataEntries.Add($attributeEntry)
$metadataEntries.Add($nameEntry)
$metadataEntries.Add($existsEntry)
$metadataEntries.Add($notExistsEntry)
$fileMetadataId = $config.AddNodeToParent($preDesktopConditions, "File Metadata")
[Void] $config.AddActionOrCondition($fileMetadataId, (New-Object EMConfigAPI.Conditions.FileMetadata("C:\test.txt", $true, $true, $metadataEntries)))
